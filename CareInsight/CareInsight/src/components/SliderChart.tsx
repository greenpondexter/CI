...
import {SliderChartPageProps} from '../containers/SliderChartContainer'

export default class SliderChart extends React.Component<SliderChartPageProps, any>{
    constructor(props: SliderChartPageProps){
        super(props);
        
    }

    componentDidMount(prevProps: SliderChartPageProps, prevState: SliderChartPageProps){
        var self = this; 
		...
		sliderChart.append("g").selectAll("miniItems")
			...
			.enter().append("rect")
			.attr("x", function(d:any) {return xScale(d.date) - 10;})
			.attr("y", (d:any) => {return yScale(d.services_count)})
			.attr("width", 10)
            .attr("height", function(d:any) {return miniHeight - yScale(d.services_count)})
			.style("fill", "red")
		...
		var line = d3.svg.line()
					.x( (d:any) => {return xScale(d.date) - 10;})
					.y( (d:any) => {return yScale1(d.risk)})
				   
		const sortedDataByDate = data.enrData.sort((x:any,y:any) => {return d3.ascending(x.date, y.date)})
        ...
        function display(){
				...
				sliderChart.select(".brush")
					.call(brush.extent([minExtent as any, maxExtent as any]));
				
				 self.props.onMemberProfileBrushUpdate([minExtent,maxExtent])
